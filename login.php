...
if (isset($_POST['login'])) {
    ...
    $stmt = mysqli_prepare($con, "SELECT id, fname, designation FROM users WHERE email=? AND password=?");
    mysqli_stmt_bind_param($stmt, 'ss', $useremail, $dec_password);
    ...
    if ($row) {
        ...
        if ($_SESSION['designation'] == 'Teacher') {
            ...
            // If the designation is neither Teacher nor Student, then there's something wrong
            die('Invalid user designation.');
        }
    } else {
